...
function createProgramForDirectory(projectRoot: string, targetDir: string): ts.Program {
...
program.getSourceFiles()
  .filter( node_modules )
  .toSorted( fileName relative from root )
*/
...
function getSortedSources(projectRoot: string, targetDir: string): readonly ts.SourceFile[] {
  const targetPath = path.join(projectRoot, targetDir);
...
describe('Printed Source Check', () => {
  test('AST of printed source should be identical to original one.', () => {
    ...
    originalSources.map<[ts.SourceFile, ts.SourceFile]>((value, index) => [value, printedSources[index]!])
      .forEach(([orginal, printed]) => {
        expect(printer.printFile(printed))
          .equals(printer.printFile(orginal));
      })
  });
});
